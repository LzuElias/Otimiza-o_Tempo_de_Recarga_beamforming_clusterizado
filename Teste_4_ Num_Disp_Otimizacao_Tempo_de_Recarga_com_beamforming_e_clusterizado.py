...
K = [10, 20, 30, 40, 50]                                  # N° dispositivos
K_dict = {i: K[i] for i in range(len(K))}  # Comprehension para criar o dicionário
N = 4                                   # N° de antenas
R = 10                                  # Raio [m]
L = 1                                   # N° de RIS
...
tempo_carregamento_total = np.zeros((len(K)))

for idx, o in enumerate(K_dict.values()):
    N_set_disp= o
    print(f"Quantidade de disp: {N_set_disp}")
    
    for p in range (0, Monte_Carlo): # --- Monte Carlo
        print(f"iteração do monte carlo: {p}")
        ...
        for n_disp in range (N_set_disp):
            ...
            x_disp = np.random.uniform(x_min, x_max, N_set_disp)
            y_disp = np.random.uniform(y_min, y_max, N_set_disp)
    ...
    #  ------------------   ELBOW METHOD PARA O N° DE CLUSTERS   -----------------------------------
        ...
        for i in range(1, N_set_disp):
            kmeans = KMeans(n_clusters = i, max_iter = 300, n_init = 10, random_state = seed)
            kmeans.fit(loc_dispositivos)
            wcss.append(kmeans.inertia_)


        for i in range(2, N_set_disp):
            clusterer = KMeans(n_clusters=i, max_iter = 300, n_init = 10, random_state = seed)
    ...
    # ---------------------   Canal Power Beacon - Dispositivos  (N_set_disp,N)   -------------------------------

        h_LoS_PB_D = np.zeros((N_set_disp,N)).astype(complex)
        h_NLoS_PB_D = np.zeros((N_set_disp,N)).astype(complex)
        Beta_PB_D = np.zeros(N_set_disp)
        hH_PB_D = np.zeros((N_set_disp,N)).astype(complex)
            # Construção do canal PB - Dispositivo (kappa_PB_D da ligação pb-disp)
        h_LoS_PB_D = np.random.rand(N_set_disp,N) + 1j*np.random.rand(N_set_disp,N) # Matriz (KxN)
        h_NLoS_PB_D = np.random.rand(N_set_disp,N) + 1j*np.random.rand(N_set_disp,N) # Matriz (KxN)
        hH_PB_D = (np.sqrt((kappa_PB_D)/(1+kappa_PB_D)) * h_LoS_PB_D) + (np.sqrt((1)/2*(1+kappa_PB_D)) * h_NLoS_PB_D) # Canal PB-Device

        
        for k in range(len(loc_dispositivos2)):
            # Beta PB_D:
            x = loc_dispositivos2[k][0]
            y = loc_dispositivos2[k][1]                     # [a][b] --> a varia de 0-N_set_disp / b=0=x / b=1=y
            d = np.sqrt(x**2 + y**2)
    ...
    # ---------------------   Canal RIS - Dispositivos Tamanho (M, N_set_disp)   ----------------------------------------

        h_LoS_RIS_D = np.zeros((M,N_set_disp)).astype(complex)
        h_NLoS_RIS_D = np.zeros((M,N_set_disp)).astype(complex)
        Beta_RIS_D = np.zeros(N_set_disp)
        hH_RIS_D = np.zeros((M,N_set_disp)).astype(complex)

            # Construção do canal RIS-Disp;
        h_LoS_RIS_D = np.random.rand(M,N_set_disp) + 1j*np.random.rand(M,N_set_disp) # Matriz (MxK)
        h_NLoS_RIS_D = np.random.rand(M,N_set_disp) + 1j*np.random.rand(M,N_set_disp) # Matriz (MxK)
    ...
    # ----------------   CÁLCULO DO TEMPO DE CARREGAMENTO DOS DISPOSITIVOS   -----------------------------------------------------------------------------------------------------
        ...
        Pr = np.zeros(N_set_disp)
        Beamform_SCSI = np.zeros((N_set_disp,N)).astype(complex)
        Gamma = np.zeros(N_set_disp)
        tempo_carregamento = np.zeros(N_set_disp)
        Gamma_kj = np.zeros(N_set_disp)

        # Beamforming S-CSI (h_LoS_PB_D)
        for k in range (0,N_set_disp):
            Beamform_SCSI[k] = np.transpose(np.sqrt(Pt/N) * (h_LoS_PB_D[k]/np.abs(h_LoS_PB_D[k])))

            #Para PB-Disp e RIS-Disp
        for k in range (0,N_set_disp):
            Phi_NEIG = np.zeros(N_set_disp)
            ...
            if k==0:
                ...
                # print(f"interação {p}")
                print(f"Tempo_carregamento: {tempo_carregamento[k]}")
            

        tempo_carregamento_total[idx] = np.sum(tempo_carregamento)
    
print(tempo_carregamento_total)

        

np.shape(K)
np.shape(tempo_carregamento_total)

# Plotando o gráfico
plt.figure(figsize=(8, 6))
plt.plot(K, tempo_carregamento_total, marker='o', linestyle='-', color='b', label='Tempo Médio de Carregamento')
...
# plt.figure(figsize=(8, 6))
# plt.plot(N_set_disp, tempo_carregamento_total, marker='o', linestyle='-', color='b', label='Tempo de Carregamento')
# plt.title("Relação entre Número de Dispositivos e Tempo de Carregamento")
# plt.xlabel("Número de Dispositivos (X)")
# plt.ylabel("Tempo de Carregamento (Y)")
# plt.grid(True)
# plt.legend()
# plt.show()
...
#   #  ------------------   ELBOW METHOD PARA O N° DE CLUSTERS   -----------------------------------
# # Mostra o Gráfico
# plt.plot(range(1, N_set_disp), wcss)
