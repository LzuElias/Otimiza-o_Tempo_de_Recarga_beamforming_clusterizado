...
for k in range (0,K):
    ...
    # Potência
    Pr = Beta @ (np.abs((Beamform_SCSI @ hH)))**2
    
    # Função Logística Tradicional (Gamma):
    Gamma = mu / (1 + (np.exp(-a*(Pr - b))))
    ...
    if k==0:
        ...
        for j in range (0, k):
            ...
            beamform_aux[j] = np.dot(Beamform_SCSI[j][:], hH[:, k])
            Gamma_kj[j] = mu / (1 + (np.exp(-a*(Beta @ ((np.abs(beamform_aux))**2)-b))))
